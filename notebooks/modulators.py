...
class FEModulator:
    def __init__(
        ...
        self.t_max = self.t_arr[
            -1
        ]  # may differ by small amount from user provided t_max.
        self.dt = dt
        ...
        for i in range(self.t_arr.size - 2, -1, -1):
            self.s_arr[:, i] = (
                ...
                + self.dt
                * self.birth_rates
                * ((1 - (1 - self.s_arr[:, i + 1]) ** 2) - self.s_arr[:, i + 1])
                + self.dt * self.death_rates * (0 - self.s_arr[:, i + 1])
                + self.dt
                * self.mutation_rates
                @ self.transition_matrix
                @ np.subtract.outer(self.s_arr[:, i + 1], self.s_arr[:, i + 1])
            )
        ...
        self.m_arr = np.sum(
            self.σ_arr
            * self.mutation_rates[np.newaxis, :, np.newaxis]
            * self.transition_matrix[:, :, np.newaxis].transpose(1, 0, 2),
            axis=0,
        )
        ...
        self.S_equispaced = np.linspace(
            self.S_arr[:, 0], self.S_arr[:, -1], len(self.t_arr)
        ).T
        self.Σ_equispaced = np.linspace(
            self.Σ_arr[:, :, 0], self.Σ_arr[:, :, -1], len(self.t_arr)
        ).transpose(1, 2, 0)
        self.M_equispaced = np.linspace(
            self.M_arr[:, 0], self.M_arr[:, -1], len(self.t_arr)
        ).T

        # Compute equispaced grids of S and Σ and M values.
        self.S_inv_arr = np.array(
            [
                [
                    np.interp(S, self.S_arr[i, :], self.t_arr)
                    for S in self.S_equispaced[i, :]
                ]
                for i in range(len(self.state_space))
            ]
        )
        self.Σ_inv_arr = np.array(
            [
                [
                    [
                        np.interp(Σ, self.Σ_arr[i, j, :], self.t_arr)
                        for Σ in self.Σ_equispaced[i, j, :]
                    ]
        ...
        self.M_inv_arr = np.array(
            [
                [
                    np.interp(M, self.M_arr[i, :], self.t_arr)
                    for M in self.M_equispaced[i, :]
                ]
                for i in range(len(self.state_space))
            ]
        )
    ...
    def _s(self, t: float, phenotype: Hashable) -> float:
        ...
        return utils.grid_interp(
            t, self.s_arr[self._idx(phenotype), :], self.t_min, self.dt
        )
    ...
    def _S(self, t: float, phenotype: Hashable) -> float:
        ...
        t_left_idx = utils.clamp(
            int(np.floor((t - self.t_min) / self.dt)), 0, len(self.t_arr) - 1
        )
        t_right_idx = utils.clamp(
            int(np.ceil((t - self.t_min) / self.dt)), 0, len(self.t_arr) - 1
        )
        t_left = self.t_min + self.dt * t_left_idx

        lin_coef = self.s_arr[self._idx(phenotype), t_left_idx]
        quad_coef = (
            self.s_arr[self._idx(phenotype), t_right_idx]
            - self.s_arr[self._idx(phenotype), t_left_idx]
        ) / self.dt

        return (
            self.S_arr[self._idx(phenotype), t_left_idx]
            + lin_coef * (t - t_left)
            + quad_coef * (t - t_left) ** 2 / 2
        )
    ...
    def _σ(self, t: float, phenotype: Hashable, phenotype2: Hashable) -> float:
        ...
        return utils.grid_interp(
            t,
            self.σ_arr[self._idx(phenotype), self._idx(phenotype2), :],
            self.t_min,
            self.dt,
        )
    ...
    def _Σ(self, t: float, phenotype: Hashable, phenotype2: Hashable) -> float:
        r"""
        Compute integral of ratio of survival probabilities of phenotype and phenotype2
        up to time t.
        """
        ...
        return self.Σ_arr[
            self._idx(phenotype), self._idx(phenotype2), t_left_idx
        ] + lin_coef * (t - t_left)
    ...
    def _m(self, t: float, phenotype: Hashable) -> float:
        ...
        return utils.grid_interp(
            t, self.m_arr[self._idx(phenotype), :], self.t_min, self.dt
        )

    def _M(self, t: float, phenotype: Hashable) -> float:
        r"""
        Compute integral of total forward equivalent mutation rate of phenotype up to
        time t.
        """
    ...
    def _S_inv(self, S: float, phenotype: Hashable) -> float:
        ...
        if S < self.S_equispaced[self._idx(phenotype), 0]:
            return (
                self.t_arr[0]
                + (S - self.S_equispaced[self._idx(phenotype), 0])
                / self.s_arr[self._idx(phenotype), 0]
            )
        elif S > self.S_equispaced[self._idx(phenotype), -1]:
            return (
                self.t_arr[-1]
                + (S - self.S_equispaced[self._idx(phenotype), -1])
                / self.s_arr[self._idx(phenotype), -1]
            )
        else:
            dS = (
                self.S_equispaced[self._idx(phenotype), 1]
                - self.S_equispaced[self._idx(phenotype), 0]
            )
            return utils.grid_interp(
                S,
                self.S_inv_arr[self._idx(phenotype), :],
                self.S_equispaced[self._idx(phenotype), 0],
                dS,
            )
    ...
    def _Σ_inv(self, Σ: float, phenotype: Hashable, phenotype2: Hashable) -> float:
        r"""
        Compute time at which integral of ratio of survival probabilities of phenotype
        and phenotype2 is Σ.
        """
        ...
        if Σ < self.Σ_equispaced[self._idx(phenotype), self._idx(phenotype2), 0]:
            return (
                ...
                + (
                    Σ
                    - self.Σ_equispaced[self._idx(phenotype), self._idx(phenotype2), 0]
                )
                / self.σ_arr[self._idx(phenotype), self._idx(phenotype2), 0]
            )
            ...
            return (
                ...
                + (
                    Σ
                    - self.Σ_equispaced[self._idx(phenotype), self._idx(phenotype2), -1]
                )
                / self.σ_arr[self._idx(phenotype), self._idx(phenotype2), -1]
            )
        else:
            dΣ = (
                self.Σ_equispaced[self._idx(phenotype), self._idx(phenotype2), 1]
            ...
            return utils.grid_interp(
                Σ,
                self.Σ_inv_arr[self._idx(phenotype), self._idx(phenotype2), :],
                self.Σ_equispaced[self._idx(phenotype), self._idx(phenotype2), 0],
                dΣ,
            )
    ...
    def _M_inv(self, M: float, phenotype: Hashable) -> float:
        r"""
        Compute time at which integral of total forward equivalent mutation rate of
        phenotype is M.
        """
        ...
        if M < self.M_equispaced[self._idx(phenotype), 0]:
            return (
                self.t_arr[0]
                + (M - self.M_equispaced[self._idx(phenotype), 0])
                / self.m_arr[self._idx(phenotype), 0]
                if self.m_arr[self._idx(phenotype), 0] > 0
                else np.inf
            )
        elif M > self.M_equispaced[self._idx(phenotype), -1]:
            return (
                self.t_arr[-1]
                + (M - self.M_equispaced[self._idx(phenotype), -1])
                / self.m_arr[self._idx(phenotype), -1]
            )
        else:
            dM = (
                self.M_equispaced[self._idx(phenotype), 1]
                - self.M_equispaced[self._idx(phenotype), 0]
            )
            return utils.grid_interp(
                M,
                self.M_inv_arr[self._idx(phenotype), :],
                self.M_equispaced[self._idx(phenotype), 0],
                dM,
            )
    ...
    def _birth_Λ_inv(self, x: Hashable, t: float, τ: float) -> float:
        r"""
        Compute Δt such that integral of birth rate of node between time node.t and
        node.t + Δt is τ.
        """
    ...
    def _mutation_Λ_inv(self, x: Hashable, t: float, τ: float) -> float:
        r"""
        Compute Δt such that integral of mutation rate of node between time node.t and
        node.t + Δt is τ.
        """
    ...
    def mutation_probs(self, node) -> np.ndarray:
        r"""
        Compute distribution of phenotype given non-birth mutation of node with
        phenotype self._attr(node) at time node.t.
        """

        phenotype = self._attr(node)
        unnormalized = self.transition_matrix[self._idx(phenotype), :] * np.array(
            [
                self._σ(node.t, child_phenotype, phenotype)
                for child_phenotype in self.state_space
            ]
        )
    ...
    def log_mutation_prob(self, node: ete3.TreeNode) -> float:
        ...
        """
        return np.log(
            self._mutation_probs(node.t, getattr(node.up, self.attr))[
                self._idx(self._attr(node))
            ]
        )
