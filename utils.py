...
def grid_interp(x: float, fp: np.ndarray, x_min: float, dx: float):
    ...
    return np.interp(x_clamp,[x_min + dx*x_left_idx,x_min + dx*x_right_idx],fp[[x_left_idx,x_right_idx]])
...
def num_subtrees_by_size(tree):
    ...
    for node in tree.traverse("postorder"):
        if node.is_leaf():
            ...
            size_by_node[node.name] = sum([size_by_node[child.name] for child in node.children])
...
def num_lineages_by_phenotype_and_time(tree, t):
    ...
    return Counter([node.state for node in tree.iter_descendants() if node.t >= t and node.up.t <= t])
...
def clade_sizes_by_phenotype(tree):
    ...
    for node in tree.traverse("postorder"):
        ...
        subclade_size_by_node[node.name] = 1 + sum([subclade_size_by_node[child.name] for child in node.children if child.state == node.state])
        if node.up is None or node.up.state != node.state: # in this case, node is the root of a subclade
            clade_sizes.append({"phenotype":node.state,"value":subclade_size_by_node[node.name]})
...
def clade_lengths_by_phenotype(tree):
    ...
    for node in tree.traverse("postorder"):
        ...
        subclade_length_by_node[node.name] = sum([child.dist for child in node.children]) + sum([subclade_length_by_node[child.name] for child in node.children if child.state == node.state]) # branch associate with parent phenotype
        if node.up is None or node.up.state != node.state: # in this case, node is the root of a subclade
            clade_lengths.append({"phenotype":node.state,"value":subclade_length_by_node[node.name]})
